...
describe('EntryPointSimulations', function () {
  ...
  describe('Simulation Contract Sanity checks', () => {
    ...
    const costInRange = (
      ...
      const max = 350
      console.log(
        `validate ${message} cost ${simCost.toNumber()} should be slightly above ep cost ${epCost.toNumber()} actual diff=${diff}`,
      )
      ...
      describe(`compare to execution ${withPaymaster} paymaster`, () => {
        ...
        it(`account verification simulation cost should be higher than execution ${withPaymaster} paymaster`, function () {
          ...
          expect(vgl).to.be.within(
            execVgl + 1,
            execVgl + diff,
            `expected simulation verificationGas to be 1..${diff} above actual, but was ${
              vgl - execVgl
            }`,
          )
        })
