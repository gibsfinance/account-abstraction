...
type Hexable = `0x${string}` | BigNumberish
...
export const createAccountOwner = (): Wallet => {
...
export const callDataCost = (data: string): number => {
  return ethers.utils
    .arrayify(data)
    .map((x) => (x === 0 ? 4 : 16))
    .reduce((sum, x) => sum + x)
...
export const calcGasUsage = async (
...
export const getAggregatedAccountInitCode = (
  entryPoint: string,
...
// becomes:
// { first: "a", second: "20" }
export const objdump = (obj: { [key: string]: any }): any => {
  if (obj == null) {
    return null
  }
...
export async function deployEntryPoint(
  provider: typeof ethers.provider = ethers.provider,
): Promise<EntryPoint> {
...
export async function createAccount(
  ...
  await (ethersSigner.provider as providers.JsonRpcProvider).send(
    'hardhat_setBalance',
    [senderCreator, toHex(100e18)],
  )
